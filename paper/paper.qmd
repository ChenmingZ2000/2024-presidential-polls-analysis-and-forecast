...
\[
\mu_i = \alpha + \beta_1 \cdot \text{methodology}_i + \beta_2 \cdot \text{days\_since\_Biden\_Withdrawal}_i + \beta_3 \cdot \text{is\_national}_i + u_{\text{state}[i]} + (v_{\text{pollster}[i]} + \gamma_{\text{pollster}[i]} \cdot \text{days\_since\_Biden\_Withdrawal}_i)
\]
...
\gamma_{\text{pollster}} \sim \text{Normal}(0, \sigma_{\text{pollster\_slope}}) \\
\end{align}
...
- \( \gamma_{\text{pollster}[i]} \) is the random slope associated with the time variable (\(\text{days\_since\_Biden\_Withdrawal}\)) for each pollster.

\[
\sigma \sim \text{Exponential}(1)
\]

We run the model in R [@citeR] using the `rstanarm` package of @rstanarm, and we use the default priors from `rstanarm`.
