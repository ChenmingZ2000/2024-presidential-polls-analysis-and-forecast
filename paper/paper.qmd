---
title: "Bayesian Hierarchical Model Predicts Dim Prospects for Trump's Victory in the 2024 U.S. Presidential Election"
subtitle: "Key Swing States Expected to Be Decisive Factors"
author: 
  - Chenming Zhao
thanks: "Code and data are available at: [https://github.com/ChenmingZ2000/2024-presidential-polls-analysis-and-forecast.git](https://github.com/ChenmingZ2000/2024-presidential-polls-analysis-and-forecast.git).
        ...
        Our survey for the 2024 U.S. presidential election is available at : [https://docs.google.com/forms/d/1GI7KoltEKEdmZNfNorXxjvapUd_s2obJyRNrMYsjndw/prefill](https://docs.google.com/forms/d/1GI7KoltEKEdmZNfNorXxjvapUd_s2obJyRNrMYsjndw/prefill)"
date: today
date-format: long
abstract: "This study uses a hierarchical Bayesian model to predict whether Donald Trump will win the 2024 U.S. presidential election based on support rates. By analyzing 2024 election data across states, pollsters, and methodologies, we capture the influence of these elements on Trump’s support rate. Our prediction shows Trump has little chance to win due to He will not have higher support rate in key swing states than Harris. This study also observes the elements shaping public sentiment and provides predictions on the dynamics of support rates in the U.S. election."
format: pdf
...
## Measurement
In the real world, people's attitudes toward public policies and political events often fluctuate, and these sentiments are frequently concealed beneath the surface. Modern society is rife with interest-driven motives and power struggles, especially at pivotal moments like the 2024 presidential election. The emotions, biases, and underlying interests of voters make capturing the true public sentiment all the more urgent. Polling thus serves as a window into the public’s mood; it is not merely a quantification of voter intent but also a battleground for election strategy and political capital.

The design process of a poll is laden with considerations. The first step is to define the measurement objective. Here, we focus on the public’s support for presidential candidates, particularly Donald Trump and Harris. However, designing a "neutral" and "scientific" poll is far from straightforward. Behind the scenes, various interest groups, aiming to sway the poll results in favor of their supported candidates, subtly influence sample selection, questionnaire design, and survey methodology, embedding "stances" within the poll itself.

When designing the poll, research teams must select an appropriate sampling frame and methodology to ensure that the data represents the broader opinions of the electorate. Yet, in a society divided by information access and technology, survey methods can introduce notable biases. For instance, face-to-face interviews or telephone surveys reach voters who are less internet-active, while online self-administered questionnaires are more likely to attract digitally savvy young people. This variation reflects not only a technical challenge but also a societal layering of bias—who gets represented, who is overlooked, whose voices are amplified, and whose are suppressed. These issues are often embedded in the survey’s design from the outset.

Next, the formal data collection phase begins. Pollsters must record each voter’s choice in an “impartial” manner, but data collection is often subject to various interferences. On one hand, respondents may not be entirely honest, particularly on sensitive political topics; on the other hand, polling organizations themselves may face influence from sponsors or political pressures, subtly adjusting questionnaires or manipulating how survey results are presented. Ultimately, public opinions are recorded as cold numbers, yet these numbers harbor significant social and political tensions.

Once data collection is complete, polling data are usually anonymized and aggregated to protect respondents’ privacy and produce “objective” statistics. However, even pre-processed data still contain random and systematic errors. This is why many media outlets and analysis platforms conduct meta-analyses of multiple polls to mitigate the bias of any single poll, aiming to capture a more accurate trend of public sentiment [@blumenthal2010polls].

The processed data are then stored in a structured database, facilitating comparisons across time, regions, and groups. On platforms like FiveThirtyEight, this data becomes the basis not only for predicting election outcomes but also for analyzing the ebbs and flows of voter sentiment. Notably, Symposium: Polls, Forecasts, and Aggregators suggests that when election day approaches, poll results often “converge” or “gravitate toward the average,” a phenomenon that may suggest that some polling organizations make final adjustments to their methodologies to present a more accurate forecast to the public and their stakeholders [@blumenthal2010polls].

Through this “game” of polling, we not only glimpse voters' preferences and mindsets but also see a microcosm of power and interest battles. Polls are more than just numbers—they are a living, breathing social portrait shaped by a multitude of forces.
...
@fig-pct3 shows the differences in Trump’s support rate distribution between national and state-level polls. National polls have a more concentrated support rate, ranging approximately between 45% and 55%, with a median slightly lower than that of state-level polls, indicating a more stable level of support across the country. In contrast, state-level polls show a wider range of support, fluctuating from around 35% to over 60%, highlighting notable variations in support across different states. Additionally, state-level polls display more outliers, further reflecting the uneven support rate among states.

```{r, message=FALSE, fig.width=6, fig.height=3}
#| label: fig-pct1
#| fig-cap: Top 5 and Bottom 5 Avg Support Rate for Donald Trump by pollsters (left) and by states with national included (right)
#| echo: false
...
```{r, message=FALSE, fig.width= 5, fig.height= 3}
#| label: fig-pct3
#| fig-cap: National support rate and state level support rate for Donald Trump 
#| echo: false

ggplot(analysis_data, aes(x = factor(is_national, levels = c(0, 1), labels = c("State Polls", "National Polls")), 
                 y = pct)) +
  geom_boxplot() +
  labs(x = "Poll Type", y = "Trump Support Rate (%)", title = "Support Rate by Poll Type (National vs. State)") +
  theme_minimal() + theme(
...
The linear component of the model is defined as follows (here $\mathrm{days\_since\_Biden\_Withdrawal}$ is shortened to DSBW): 

$$
\mu_i = \alpha + \beta_1 \cdot \text{methodology}_i+ \beta_2 \cdot \mathrm{DSBW}_i \\
\\
       + \beta_3 \cdot \mathrm{is\_national}_i + u_{\text{state}[i]} + (v_{\text{pollster}[i]} + \gamma_{\text{pollster}[i]} \cdot \mathrm{DSBW}_i) \\
$$
...
#| eval: true
#| label: tbl-model
#| tbl-cap: Model summary table for model which predicts support rate for Donald Trump in 2024 presidential election 
#| warning: false
#| message: false
...
## Prediction Output
The swig states information is from @270toWin. @fig-results shows that states with similar demographic characteristics (such as Pennsylvania and Michigan) exhibit comparable fluctuations in support rates. It also indicates that the model predicts Trump’s national support rate on Election Day, November 5, 2024, to center around 45% to 50%, with moderate variation. This suggests that despite the influence of national events and polling method differences, the national support rate remains relatively stable, reflecting a steady stance among most voters. We forecast that Trump’s national average support rate will be below 50%. At the state level, @fig-results shows variations in support rates among swing states, with swing states other than Arizona and North Carolina tending to show average support rates below 50%. Swing states like Pennsylvania and Michigan display significant volatility, as indicated by wider interquartile ranges and multiple outliers in the box plot. This volatility suggests that voter preferences in these states are less stable, and support rates in competitive states are more susceptible to minor events or policy shifts.
...
swing_states <- c("Arizona", "Nevada", "Michigan", "Wisconsin", "Pennsylvania", "North Carolina", "national")
need_states <- prediction_data[prediction_data$state %in% swing_states, ]

# Plot for state data
ggplot(need_states, aes(x = state, y = predicted_support)) +
  geom_boxplot(color = "lightblue") +  # Adds slight horizontal jitter to reduce overlap
  ...
  labs(title = "Election Day Trump Support Rate Prediction Both Nation and Swing States Icluded", y = "Predictied Support Rate (%)", x = "States") +
  theme_minimal() +
...
Visualizations often struggle to fully capture the complex structure of our model. The model includes data from 50 U.S. states, multiple polling organizations, and various survey methodologies. If we attempted to display every detail within a chart, the visualizations would become overwhelmingly complex, making it difficult for readers to read or understand. Conversely, if we limit the display to a subset, such as only highlighting key states or selected polling methods, this approach risks losing the completeness of information. These simplified visualizations fail to convey all sources of variation within the model, potentially leading readers to overlook significant factors and misunderstand the impact of omitted influences. Additionally, this partial representation can foster a feedback loop where oversimplified summaries influence potential voters' perceptions, impacting the trends in unpredictable ways.
...
### Polling Methods
Imagine you are watching an orchestra, hoping it will perform seamlessly. Every instrument needs to be tuned to just the right pitch so that it combines into a harmonious melody, capturing the audience’s attention and drawing them in. This is exactly how polling works—crafting the notes of public opinion into a clear melody that aims to capture and showcase the true views of American voters. In this appendix, we focus on The New York Times and Siena College, a pollster that has consistently received the highest rating of 3.0 in recent years from @fivethirtyeight, as the subject of our study and discussion.

Before diving into the specific sampling methods and results, we consider the poll's target population, framework, and sample selection. The dataset used in the appendix is from @fivethirtyeight. The dataset includes poll results from the middle of 2022 to the present. The choice of target population and sample directly impacts the accuracy and relevance of poll results. @tbl-polls1 provides an overview of several key components of The New York Times and Siena College poll data. From this table, we see that their target population is registered voters (RV), with particular attention given to those likely to vote in the election—likely voters (LV). This choice is guided by the goal of predicting election results by focusing on the voter groups that have the greatest influence. In actual elections, those with the highest intention to vote often impact the results the most. By selecting and focusing on those with strong voting intentions, the poll seeks to ensure that the results are more representative and predictive. By concentrating on these "likely voters," the poll aims to mirror the actual composition of voters, providing a forecast closer to the election scenario.
...
ny_data <- read_parquet(here :: here("data/02-analysis_data/pollster_data.parquet"))
```
...
kable(head(ny_data, 6), 
      col.names = c("pollster_name", "538_numeric_grade", "methodology", "population"), 
      align = c("l", "c", "c", "c")) %>%
...
After selecting the target population, the next step is determining the polling framework. From @nytimes2018pollmethod, we can see that the phone numbers for The New York Times and Siena College poll samples come from national voter files and market research companies. Specifically, the polling team collaborates with L2, extracting samples from the national voter file and supplementing them with matching cell phone numbers provided by Marketing Systems Group to create a database of phone contacts. The purpose of these dual information sources is to allow the poll to gather more complete contact information. Once the phone numbers are obtained, the polling team randomly dials both cell phones and landlines, using the different contact methods to reach diverse groups. Combined, these methods provide a broader, more inclusive view of public opinion. This process is known as a dual-frame approach.

In sampling, The New York Times and Siena College employ stratified sampling, dividing voters by characteristics such as race, party affiliation, and region [see @nytimes2024methodology]. The purpose of stratified sampling is to ensure that each significant voter subgroup is appropriately represented in the final sample, capturing the diversity of the population and preventing the poll from lacking representativeness or being biased. Imagine a jigsaw puzzle, where each group is like a puzzle piece that must fit perfectly within the whole to form a complete picture. Of course, finding the right "pieces" and placing them in the correct spots requires careful planning. The polling team relies on accurate and up-to-date demographic data, a challenge that demands extensive resources and attention to detail.

Then there is population weighting and the voter likelihood model. To make the sample more reflective of the actual voting population, the team takes into account past voting behavior, demographic characteristics, and respondents' self-reported voting intention. This adjustment gives more weight to the feedback from those who are likely to vote. In essence, this model fine-tunes the sample to better align with real voting patterns. However, much like a conductor shaping a new season’s performance based on the last, this model relies on past data, which may overlook new trends or unexpected changes in behavior [see @nytimes2018pollmethod].

Finally, it goes to question design. Like lyrics in a song, each question needs to encourage respondents to share their genuine opinions, avoiding wording that could sway answers. The New York Times and Siena College publish every question verbatim, allowing anyone to see exactly what was asked. Like an open rehearsal, this transparency builds trust. The questions are clear and aim to remain neutral, though the simplicity can sometimes limit depth, missing subtle aspects in complex topics.

### Challenges and New York Times' Response
One challenge that arises in polling is that, despite the careful design aimed at ensuring broad coverage, not everyone is willing to participate. This is known as the **nonresponse issue**. To make the results more representative, pollsters at The New York Times and Siena College make multiple attempts to contact individuals. However, despite these efforts, the overall response rate remains around 2%, which is relatively low. This low rate naturally raises curiosity about how those who stay silent might view the issues. In 2022, an experiment aimed at measuring the impact of nonresponse on their phone surveys suggested that the resulting bias might not be substantial. However, The New York Times also acknowledges, “the truth is that there’s no way to be absolutely sure that the people who respond to surveys are like demographically similar voters who don’t respond. It’s always possible that there’s some extra dimension of nonresponse that we haven’t considered” [see @nytimes2024methodology].

We are concerned about how feedback from around 1,000 people can represent the entire United States. To address this question, The New York Times uses the analogy of “tasting a soup with just one spoonful.” They argue that their method includes a sufficiently random sample, noting that if they were to double—or even triple—the number of respondents, the margin of error would only slightly decrease, possibly by about two percentage points. In other words, increasing the sample size would not greatly improve overall accuracy. The New York Times explains that once the survey is complete, they compare respondents to voter files and use a process known as weighting to ensure the sample reflects the broader voting population. In practice, this often means giving more weight to groups less likely to participate in surveys, such as those without a college degree [@nytimes2024methodology] [@nytimes2022wisconsin].

Then there’s the voter likelihood model. In the NYT/Siena poll, they use information from voter files to review each voter’s voting history, using past behavior to predict future voting patterns. They also ask respondents about their likelihood to vote and use this information to estimate how likely someone is to vote in the upcoming election. However, much like a conductor adjusts a new season’s performance based on the previous one, this model relies on past data, which might miss new trends or unexpected changes in behavior. The New York Times itself mentions that the margin of error in its polls ranges between 2% and 4%, a factor that could play a decisive role in a close election [see @nytimes2024methodology].

Additionally, even with the best planning, the New York Times notes that they ensure the correct number of interviews by race, party, and region so that each NYT/Siena poll reaches, for example, the appropriate proportion of white Democrats from the West, or Hispanic Republicans from Maricopa County, Arizona [@nytimes2024methodology] [@nytimes2022wisconsin]. Still, some groups remain difficult to reach. Pollsters attempt to compensate for this by adjusting sample weights, but weighting itself introduces challenges. If certain groups are over- or under-adjusted within the sample, it may destabilize results and obscure potential sample limitations.

### The Advantages and Disadvantages of the Questionnaire Design

From polls like @nytimes2024texaspoll, the NYT/Siena polls maintain a high level of transparency, with each survey report publishing the questionnaire verbatim. This approach allows both the public and researchers to clearly understand the questions asked, enhancing trust. The questions are also straightforward, avoiding complex wording, which reduces the risk of misunderstandings across different respondent groups and improves response accuracy. Furthermore, the language is kept neutral, avoiding any leading phrases. This neutrality helps prevent bias that might arise from the phrasing of questions, ensuring that respondents’ answers are based on their genuine views rather than being influenced by question design, which supports the objectivity of the results.

The questionnaire is designed to examine respondents’ multifaceted characteristics, covering not only basic demographics like age, gender, and education level but also other personal traits and preferences. This information helps analyze political opinion differences across diverse backgrounds, making the results more representative. For example, the questionnaire may include questions on race, occupation, or top priorities when choosing a president—factors that often significantly impact voting preferences. It also includes questions about respondents' group characteristics, such as geographic location, community type (urban or rural), and political affiliation. By covering these factors, the survey can accurately reflect differences in public opinion across different regions and social environments. In a politically polarized climate, this detailed grouping helps identify regional or community-specific trends, showing how voters from different backgrounds feel about candidates or policies.

The questionnaire also includes questions about religious beliefs to analyze the impact of religion on voting intentions. Religious affiliation often aligns closely with political views, particularly in the U.S., where religious beliefs can strongly influence positions on morality, social policies, and party preferences. By incorporating religious affiliation, the questionnaire can better capture how religion influences voter choices, allowing for more targeted analysis of various religious backgrounds.

However, certain aspects of the questionnaire design have limitations. For instance, to keep questions concise, the survey may not delve deeply into complex topics, possibly overlooking subtle but important perspectives. For example, questions about voting likelihood are sometimes too general; each respondent may interpret “likelihood” differently based on their personal standards. Additionally, the lack of open-ended questions limits the depth of responses, reducing the questionnaire's ability to capture detailed opinions.

Another limitation is the focus on mainstream religions, often overlooking other groups such as Islam, Hinduism, Buddhism, and Sikhism. With increasing immigration, the number of voters from these religious backgrounds is rising, and their political preferences may differ from those of traditional religious groups. For instance, Muslim, Hindu, or Buddhist voters may prioritize issues such as cultural diversity, foreign policy, or religious freedom differently. The absence of questions covering these religions may result in a poll that does not fully reflect the priorities of all voters, limiting its representativeness. It also makes it harder to track the growing influence and political leanings of these groups. Without including these religious backgrounds, the questionnaire may miss important shifts in political attitudes among these emerging voter groups, potentially losing opportunities for future research.

The question in The Times/Siena Poll: Texas Topline, “If you had to decide between the two today, would you lean more toward” saw a high rate of “Don’t know/Refused” responses, at 66% [see @nytimes2024texaspoll]. This likely reflects a sense of dissatisfaction among voters due to political polarization. Many voters may feel discontent with both major candidates (Trump and Harris) and, as a result, choose not to respond when dissatisfied with the options or disillusioned with the political landscape. Additionally, the phrasing, “if you had to decide between the two today,” may make respondents feel pressured, leading to a refusal to answer. For those inclined toward third-party candidates or undecided voters, this type of question may further discourage responses. This question specifically targeted “leaners” (respondents who previously did not clearly support either Trump or Harris), often representing “swing” or “undecided” voters. Asking these voters, who are inherently unsure, to make a decision without strong support for either candidate could contribute to the high rate of refusals. A similar pattern is observed in questions like “(Leaners, if no candidate selected in initial multi-candidate question) If you had to decide today, would you lean more toward,” where respondents also frequently chose “Don’t know/Refused.”

# Methodology and Survey Design

To design an effective survey method, we will begin by defining our target population and sampling framework. Our survey will target both **likely voters (LV)**, those who have a high intention of voting, and **registered voters (RV)**. Likely voters provide more accurate election predictions by focusing on those most likely to participate. However, registered voters, including those with lower voting intentions, also represent broader social and political attitudes. Including them ensures the survey reflects not only who is likely to win but also captures societal perspectives on what defines a presidential candidate.

To ensure representativeness, we will use **stratified random sampling**. The sample will be divided by age, race, education, income level, and region. This approach allows selective, representative sampling within key groups, reducing sampling bias and improving reliability.

With a budget of $100,000, we plan to survey approximately 1,500 respondents, covering these demographic groups. Based on The New York Times' experience, this sample size is sufficient to represent the overall picture of the United States [see @nytimes2024methodology]. We aim to keep the margin of error around 2%-4% while ensuring the representativeness of each subgroup, creating a snapshot of U.S. society within the election context. 

Based on the polling methodology of [@nytimes2024methodology], we’ll combine email and phone outreach and provide small incentives (about five dollars to ten dollars) to encourage participation and reduce non-response problem. Additionally, we plan to work with trusted panel providers, such as YouGov or Dynata, using certified voter panels that align with our target population.

For the survey itself, we will use Google Forms as the platform. Familiarity with this platform makes respondents less likely to resist or question the survey. It offers ease of access, cost-effectiveness, and reliable data collection. Questions will be crafted with clear and neutral wording, allowing respondents to understand the questions without any influence from suggestive language. The survey will mostly consist of closed-ended questions and scaled questions to quantify preferences, making the data easier to analyze. Additionally, the survey will include one or two open-ended questions, allowing respondents to share their main concerns or factors influencing their choices. This setup captures responses that are difficult to summarize or quantify but are important for context.

After data collection, we will proceed with data cleaning. First, responses will be cross-verified with national voter files to ensure that respondents are registered voters, filtering out responses from non-voters. Each response will be tagged with a unique identifier (such as survey ID) to prevent duplicates. After cleaning, we will apply weighting based on age, race, education, and region to align the sample structure with the overall voting population.

Our budget will be allocated as follows:

- Approximately $50,000 for panel partnerships and respondent incentives.

- Around $15,000 for voter file cross-verification, data cleaning, and weighting.

- $20,000 for poll aggregation, regression modeling, and consultation with election analysts.

- $15,000 for staffing, platform fees, and survey testing.

Our survey is available [here](https://docs.google.com/forms/d/1GI7KoltEKEdmZNfNorXxjvapUd_s2obJyRNrMYsjndw/prefill)
